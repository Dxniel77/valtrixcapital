...
export async function POST(req: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Invalid trader selection" }, { status: 400 });
  }
  ...
  try {
    const result = await deleteAdminCopyTraders(parsed.data.ids, admin.id);
