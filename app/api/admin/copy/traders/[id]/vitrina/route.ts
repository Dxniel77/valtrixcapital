...
import { adminCopyVitrinaSchema } from "@/lib/copy-trading/admin-schema";
import { isDatabaseAvailable } from "@/lib/db/available";
import {
  CopyTradingError,
  updateAdminTraderVitrina,
...
export async function PATCH(req: Request, { params }: Params) {
  ...
  const parsed = adminCopyVitrinaSchema.safeParse(
  ...
  try {
    ...
    const trader = await updateAdminTraderVitrina(id, parsed.data, admin.id);
